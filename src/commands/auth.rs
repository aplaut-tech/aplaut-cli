...
fn login(ctx: &Ctx) -> Result<(), CliError> {
    ...
    let base_url = ctx.global.base_url.as_deref().map(resolve::validate_base_url).transpose()?;
    let token = read_token(ctx)?;
    let paths = Paths::resolve(&ctx.env)?;
    let mut credentials = config::load_credentials(&paths, &ctx.reporter)?;
    credentials
        .profiles
        .insert(profile.clone(), ProfileCredentials { access_token: token.expose().to_string() });
    ...
    ctx.reporter.info(&format!("Токен сохранён в профиль «{profile}» ({}).", paths.credentials.display()));
    if let Some(url) = base_url {
        ctx.reporter.info(&format!("Base URL профиля: {url}"));
    }
    ctx.reporter.info("Токен не проверялся на сервере: проверка появится вместе с `aplaut auth status`.");
    Ok(())
}
...
fn logout(ctx: &Ctx) -> Result<(), CliError> {
    ...
    if credentials.profiles.remove(&profile).is_none() {
        ctx.reporter.info(&format!("В профиле «{profile}» нет сохранённого токена — удалять нечего."));
        return Ok(());
    }
    config::save_credentials(&paths, &credentials)?;
    ctx.reporter.info(&format!("Токен профиля «{profile}» удалён из {}.", paths.credentials.display()));
    Ok(())
}
