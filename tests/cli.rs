...
fn one_page() -> Reply {
    Reply::json(200, first_page_json(&[review("r1", "t1")], None, false, 1, None))
}

fn scroll<'a>(resource: &'a str, base_url: &'a str, extra: &[&'a str]) -> Vec<&'a str> {
    let mut args = vec![resource, "scroll", "--filter", FILTER, "--base-url", base_url];
...
fn no_args_prints_help_with_examples_and_exits_2() {
    ...
    assert_eq!(out.code, 2);
    assert!(out.stderr.contains("Примеры:") && out.stderr.contains("reviews") && out.stderr.contains("support@aplaut.com"), "{}", out.stderr);
}
...
fn help_subcommand_and_long_help_lead_with_examples() {
    ...
    assert_eq!(out.code, 0, "{}", out.stderr);
    assert!(out.stdout.contains("--filter") && out.stdout.contains("Примеры:") && out.stdout.contains("30 дней"));
}
...
fn typo_suggests_and_is_json_without_tty() {
    ...
    assert!(err["error"]["message"].as_str().unwrap().contains("reviews"), "{err}");
}
...
fn invalid_include_fails_before_any_request() {
    ...
    let out = aplaut(home.path(), &scroll("reviews", &url, &["--include", "nope"]), &[("APLAUT_ACCESS_TOKEN", "tok")], "");
    assert_eq!(out.code, 2);
    let err = out.error_json();
    assert_eq!((err["error"]["code"].as_str(), err["error"]["field"].as_str()), (Some("invalid_include"), Some("include")));
...
fn remote_http_base_url_is_rejected() {
    ...
    let out = aplaut(home.path(), &scroll("reviews", "http://example.com/v4", &[]), &[("APLAUT_ACCESS_TOKEN", "tok")], "");
    assert_eq!(out.code, 2);
...
fn scroll_jsonl_end_to_end_with_state() {
    ...
    let server = MockServer::start(vec![
        Reply::json(200, first_page_json(&[review("r1", "t1"), review("r2", "t2")], Some("c1"), true, 3, None)),
    ...
    let out = aplaut(home.path(), &scroll("reviews", &url, &["--format", "jsonl", "--state", state_arg]), &[("APLAUT_ACCESS_TOKEN", "tok")], "");
    assert_eq!(out.code, 0, "{}", out.stderr);
    assert_eq!(out.stdout.lines().count(), 3);
    assert!(out.stderr.contains("обход завершён"), "{}", out.stderr);
    let saved: serde_json::Value = serde_json::from_str(&fs::read_to_string(&state).unwrap()).unwrap();
    assert_eq!((saved["completed"].as_bool(), saved["emitted"].as_u64()), (Some(true), Some(3)));
...
fn default_format_is_raw_one_line_per_page() {
    ...
    let url = server.base_url();
    let out = aplaut(home.path(), &scroll("products", &url, &[]), &[("APLAUT_ACCESS_TOKEN", "tok")], "");
    assert_eq!(out.code, 0, "{}", out.stderr);
...
fn csv_format_writes_header_and_rows() {
    ...
    let server = MockServer::start(vec![Reply::json(200, first_page_json(&[review("r1", "t1"), review("r2", "t2")], None, false, 2, None))]);
    let url = server.base_url();
    let out = aplaut(home.path(), &scroll("reviews", &url, &["--format", "csv"]), &[("APLAUT_ACCESS_TOKEN", "tok")], "");
    assert_eq!(out.code, 0, "{}", out.stderr);
    let lines: Vec<&str> = out.stdout.lines().collect();
    assert_eq!(lines[0], "id,type,body,rating,updated_at,author_ref,product_ref");
...
fn partial_failure_exits_4_with_flushed_stdout() {
    ...
    let server = MockServer::start(vec![
        Reply::json(200, first_page_json(&[review("r1", "t1"), review("r2", "t2")], Some("c1"), true, 9, None)),
        Reply::Hangup,
    ]);
    let url = server.base_url();
    ...
    let out = aplaut(
        home.path(),
        &scroll("reviews", &url, &["--format", "jsonl", "--state", state.to_str().unwrap(), "--max-retries", "0"]),
        &[("APLAUT_ACCESS_TOKEN", "tok")],
        "",
    );
    assert_eq!(out.code, 4, "{}", out.stderr);
    assert_eq!(out.stdout.lines().count(), 2, "страница до сбоя должна быть в stdout");
    let err = out.error_json();
    assert_eq!((err["ok"].as_bool(), err["error"]["code"].as_str()), (Some(false), Some("network_error")));
    let saved: serde_json::Value = serde_json::from_str(&fs::read_to_string(&state).unwrap()).unwrap();
...
fn closed_stdout_stops_with_partial_exit() {
    ...
    let server = MockServer::start(vec![
        Reply::json(200, first_page_json(&[review("r1", "t1")], Some("c1"), true, 3, None)),
        Reply::json(200, page_json(&[review("r2", "t2")], Some("c2"), true)),
        Reply::json(200, page_json(&[review("r3", "t3")], None, false)),
    ]);
    let url = server.base_url();
    let state = home.path().join("s.json");
    let args = scroll("reviews", &url, &["--format", "jsonl", "--state", state.to_str().unwrap()]);
    ...
    let saved: serde_json::Value = serde_json::from_str(&fs::read_to_string(&state).unwrap()).unwrap();
    assert_eq!(saved["cursor"], "c1", "стейт не должен уйти дальше отданного");
}
...
fn verbose_never_leaks_token() {
    ...
    let server = MockServer::start(vec![
        one_page(),
        Reply::text(401, "").with_header("WWW-Authenticate", r#"Bearer realm="Doorkeeper", error="invalid_token""#),
    ]);
    let url = server.base_url();
    let ok = aplaut(home.path(), &scroll("reviews", &url, &["--verbose"]), &[("APLAUT_ACCESS_TOKEN", token)], "");
    let denied = aplaut(home.path(), &scroll("reviews", &url, &["--verbose"]), &[("APLAUT_ACCESS_TOKEN", token)], "");
    assert_eq!((ok.code, denied.code), (0, 3));
    for out in [&ok, &denied] {
        assert!(!out.stdout.contains(token) && !out.stderr.contains(token), "{}", out.stderr);
    }
...
fn quiet_hides_summary() {
    ...
    let server = MockServer::start(vec![one_page()]);
    let url = server.base_url();
    let out = aplaut(home.path(), &scroll("reviews", &url, &["-q"]), &[("APLAUT_ACCESS_TOKEN", "tok")], "");
...
fn login_from_stdin_then_scroll_uses_profile_token_and_base_url() {
    ...
    let server = MockServer::start(vec![one_page()]);
    let url = server.base_url();
    let login = aplaut(home.path(), &["auth", "login", "--token-stdin", "--profile", "ci", "--base-url", &url], &[], "tok-ci\n");
    ...
    assert!(fs::read_to_string(home.path().join("config/aplaut/config.toml")).unwrap().contains(&url));
    assert!(!login.stderr.contains("tok-ci"));
    let out = aplaut(home.path(), &["reviews", "scroll", "--filter", FILTER, "--profile", "ci"], &[], "");
    assert_eq!(out.code, 0, "{}", out.stderr);
    assert_eq!(server.requests()[0].header("authorization"), Some("Bearer tok-ci"));
}
...
fn profile_flag_beats_env_token_and_token_file_env_beats_token_env() {
    ...
    let url = server.base_url();
    assert_eq!(aplaut(home.path(), &["auth", "login", "--token-stdin", "--profile", "ci"], &[], "tok-ci").code, 0);
    let file = home.path().join("token");
    fs::write(&file, "file-tok\n").unwrap();
    let a = aplaut(home.path(), &scroll("reviews", &url, &["--profile", "ci"]), &[("APLAUT_ACCESS_TOKEN", "env-tok")], "");
    let b = aplaut(home.path(), &scroll("reviews", &url, &[]), &[("APLAUT_ACCESS_TOKEN", "env-tok"), ("APLAUT_ACCESS_TOKEN_FILE", file.to_str().unwrap())], "");
...
fn logout_removes_token_and_is_idempotent() {
    ...
    assert_eq!(aplaut(home.path(), &["auth", "login", "--token-stdin"], &[], "tok").code, 0);
    let first = aplaut(home.path(), &["auth", "logout"], &[], "");
    let second = aplaut(home.path(), &["auth", "logout"], &[], "");
    assert_eq!((first.code, second.code), (0, 0));
    assert!(!fs::read_to_string(home.path().join("config/aplaut/credentials")).unwrap().contains("tok"));
...
fn state_file_for_other_resource_is_usage_error() {
    ...
    assert_eq!(aplaut(home.path(), &scroll("reviews", &url, &["--state", s]), &[("APLAUT_ACCESS_TOKEN", "tok")], "").code, 0);
    let out = aplaut(home.path(), &scroll("products", &url, &["--state", s]), &[("APLAUT_ACCESS_TOKEN", "tok")], "");
    assert_eq!(out.code, 2);
