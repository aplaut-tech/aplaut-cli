...
На macOS бинарь из архива, скачанного браузером, Gatekeeper не запустит, пока не снять карантин:
`xattr -d com.apple.quarantine aplaut`. С `curl | sh` и `self update` этого не нужно.
...
```json
{"ok":true,"command":"profile.set","cli_version":"0.3.0","dry_run":false,
 "result":{"profile":"staging","created":true,"changes":[{"field":"base_url","from":null,"to":"https://…"}],"config_path":"…"},
 "warnings":[]}
{"ok":false,"command":"reviews.scroll","cli_version":"0.3.0","dry_run":false,
