...
/// повторяется POST (идемпотентности у API нет) и PUT, повтор которого адресовал бы уже другой
/// объект (смена external_id); обычный PUT идемпотентен и повторяется как GET.
